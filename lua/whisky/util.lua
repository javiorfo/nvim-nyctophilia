...
local function executeAll(tbl, fn)
...
function M.get_hex_color(xterm_nr)
...
-- Function to link group to style
-- @param group. Ex: "Comment"
-- @param highlight. Ex: "String"
function M.link(group, highlight)
    local link = string.format("hi link %s %s", group, highlight)
    vim.cmd(link)
end

-- Function to link all
-- @param links. Ex: { Statement = "Type", Conditional = "Normal" }
function M.linkAll(links)
    executeAll(links, M.link)
end
...
function M.highlight(group, options)
    ...
    if cterm_fg ~= "NONE" then
        fg = select(2, M.get_hex_color(options.fg))
    end

    local bg = "NONE"
    if cterm_bg ~= "NONE" then
        bg = select(2, M.get_hex_color(options.bg))
    end
...
-- Function to hightlight all
-- @param links. Ex: { Statement = { fg = 15, bg = 0, gui = "bold" } }
function M.highlightAll(links)
    executeAll(links, M.highlight)
end

-- Function to sync a keyword with N matches
-- @param keyword_name. Ex: "javaImport"
-- @param ... Ex: "import", "package"
function M.synKeyword(keyword_name, ...)
    local result = "syn keyword " .. keyword_name
